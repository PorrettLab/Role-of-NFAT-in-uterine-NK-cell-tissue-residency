---
title: "Inhibition of NFAT promotes loss of tissue resident uterine 1 natural killer cells and attendant pregnancy complications in humans - Integrated Analysis of HC and UTx Datasets - QC"
author: "RA"
output:
...
UTx03_Bx04
head(UTx03_Bx04@meta.data)
UTx03_Bx04@assays

UTx04_Bx01
head(UTx04_Bx01@meta.data)
UTx04_Bx01@assays
```


Given some samples have two assays, we need to specify which assay we are working with at a time like so, for this run,  we'll set the assay to RNA:
```{r}
...
DefaultAssay(UTx03_Bx04) <- 'RNA' 
DefaultAssay(UTx04_Bx01) <- 'RNA' 
```
...
```{r,fig.height=7, fig.width=13}
#We'll store the percentage of reads that map to the mitochondrial genome in the metadata object as "percent.mt"
UTx01_Bx01[["percent.mt"]] <- PercentageFeatureSet(UTx01_Bx01, pattern = "^MT-")

#The number of unique genes and total molecules are automatically calculated during `CreateSeuratObject()` and we can find these stored in the object meta data as nFeature_RNA and nCount_RNA respecitvely.
UTx01_Bx01@meta.data %>%
  head(n=5)
```
...
```{r, message=FALSE, fig.height=4}
#Visualize QC metrics as violin plots
p1 <- VlnPlot(UTx01_Bx01, features = c("nFeature_RNA"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
p2 <- VlnPlot(UTx01_Bx01, features = c("nCount_RNA"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
p3 <- VlnPlot(UTx01_Bx01, features = c("percent.mt"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
grid.arrange(p1, p2, p3, ncol=3)
```
...
```{r, message=FALSE, fig.width=20, fig.height=7}
#FeatureScatter is typically used to visualize feature-feature relationships, but can be used for anything calculated by the object, i.e. columns in object metadata, PC scores etc.
plot1 <- FeatureScatter(UTx01_Bx01, feature1 = "nCount_RNA", feature2 = "percent.mt", cols = "gray60")
plot2 <- FeatureScatter(UTx01_Bx01, feature1 = "nCount_RNA", feature2 = "nFeature_RNA", cols = "gray60")
plot1 + plot2
```

**Alternative view of the data and distributions**
```{r, fig.height=3}
df <- as.data.table(UTx01_Bx01@meta.data)
sel <- c("orig.ident", "nCount_RNA", "nFeature_RNA", "percent.mt")
df <- df[, sel, with = FALSE]
df[1:3, ]
fontsize <- 10
...
### More on Data and QC
```{r, message=FALSE, warning=FALSE, fig.height=4, fig.width=6}
counts <- Matrix(UTx01_Bx01@assays$RNA@counts)
counts_per_cell <- Matrix::colSums(counts)
counts_per_gene <- Matrix::rowSums(counts)
genes_per_cell1 <- Matrix::colSums(counts>0) #count a gene only if it has non-zero reads mapped.
cells_per_gene <- Matrix::rowSums(counts>0) #only count cells where the gene is expressed
...
```{r fig.width=12, fig.height=6}
#Density plot
ggplot(UTx01_Bx01@meta.data, aes(x=UTx01_Bx01@meta.data$percent.mt)) +
  geom_density() + scale_color_manual(values=c("blue")) + theme_classic() +
  geom_vline(aes(xintercept=mean(UTx01_Bx01@meta.data$percent.mt)),
             color="blue", linetype="dashed", size=0.5) +scale_x_continuous(breaks=seq(0, 40, by = 1))
```
...
```{r}
(Count93_nCount_RNA <- quantile(UTx01_Bx01@meta.data$nCount_RNA, 0.93)) # calculate value in the 93rd percentile for a hint on thresholds but these should be taken with a grain of salt, look at the above plots as well to determine thresholds
(Count93_nFeature_RNA <- quantile(UTx01_Bx01@meta.data$nFeature_RNA, 0.93))
(Count93_percent_mt <-  quantile(UTx01_Bx01@meta.data$percent.mt, 0.93))

summary(UTx01_Bx01@meta.data$nCount_RNA)
summary(UTx01_Bx01@meta.data$nFeature_RNA)

UTx01_Bx01 <- subset(UTx01_Bx01, subset = nFeature_RNA > 200 & nFeature_RNA < 3500 & percent.mt < 10)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx01/UTx01_Bx01_doublet_predictions.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx01/UTx01_Bx01_doublet_predictions_histogram.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```

```{r out.height = "460px", out.width='800px', echo=F, eval=FALSE}
knitr::include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx01/UTx01_Bx01_doublet_predictions_histogram.pdf')
```

```{r}
#Loading scrublet predictions
dim(scrublet_calls <- read.csv("/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx01/UTx01_Bx01_scrublet_calls.csv"))
table(scrublet_calls$predicted_doublets)

scrublet_calls$X <- paste0("Pre-Cycle-Resumption_UTx01_Bx01_", scrublet_calls$X)

dim(scrublet_calls <- scrublet_calls[which(scrublet_calls$X %in% rownames(UTx01_Bx01@meta.data)),])
rownames(scrublet_calls) <- scrublet_calls$X
scrublet_calls$X <-NULL
dim(scrublet_calls)

#Adding doublet information to metadata
#First we'll ensure that the rownames in UTx01_Bx01 match the rownames in scrublet_calls. AddMetaData maps rownames but we'll still do so to ensure that mapping of predictions are made to respective bar codes
scrublet_calls <- scrublet_calls[rownames(UTx01_Bx01@meta.data), ]
head(rownames(scrublet_calls))
head(rownames(UTx01_Bx01@meta.data))
UTx01_Bx01 <- AddMetaData(UTx01_Bx01, scrublet_calls)
```


```{r, fig.height=4, fig.width=5}
#Without normalizing the data, we want to first visualize the doublets in our datasets
UTx01_Bx01_Control_2 <- UTx01_Bx01
UTx01_Bx01_Control_2 <- FindVariableFeatures(UTx01_Bx01_Control_2, selection.method = "vst", nfeatures = 2500)
UTx01_Bx01_Control_2 <- ScaleData(object = UTx01_Bx01_Control_2, scale.max = 30,  verbose = FALSE)
UTx01_Bx01_Control_2 <- RunPCA(object = UTx01_Bx01_Control_2, npcs = 30, verbose = FALSE)
UTx01_Bx01_Control_2 <- FindNeighbors(UTx01_Bx01_Control_2, dims = 1:20, verbose = TRUE, reduction = "pca")
UTx01_Bx01_Control_2 <- RunUMAP(UTx01_Bx01_Control_2, dims = 1:20, verbose = TRUE, reduction = "pca")
UTx01_Bx01_Control_2 <- FindClusters(UTx01_Bx01_Control_2, verbose = TRUE, reduction = "pca") #Resolution can be adjusted - leaving to default for now in test dataset

FeaturePlot(UTx01_Bx01_Control_2, features = "doublet_scores", pt.size = 0.01)

DimPlot(UTx01_Bx01_Control_2, group.by = "predicted_doublets", pt.size = 0.01, cols = c("gray90", "firebrick3"))

#Checking the nUMI for doublets and singlets
VlnPlot(UTx01_Bx01_Control_2,
        features = "nCount_RNA",
        pt.size = 0,
        group.by = "predicted_doublets") + NoLegend()

#Fractions of doublets per cluster
df <- data.table(UTx01_Bx01_Control_2@meta.data)
...
```{r, fig.height=4, fig.width=5}
#Next we'll remove the doublets and see what the data looks like
UTx01_Bx01_Control_2 <- UTx01_Bx01_Control_2[, UTx01_Bx01_Control_2@meta.data[, "predicted_doublets"] == "False"]
unique(UTx01_Bx01_Control_2@meta.data$predicted_doublets)
DimPlot(UTx01_Bx01_Control_2, group.by = "predicted_doublets", pt.size = 0.01, cols = c("gray90", "firebrick3"), label = TRUE)
```

**Filtering cells to remove doublets**
```{r}
UTx01_Bx01 <- UTx01_Bx01[, UTx01_Bx01@meta.data[, "predicted_doublets"] == "False"]
unique(UTx01_Bx01@meta.data$predicted_doublets)
dim(UTx01_Bx01)
```
...
### Saving object for further downstream analysis
```{r}
saveRDS(UTx01_Bx01, file = "/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/QC/UTx01_Bx01_SeuratObject_soupX_doublet_QC_filtered.rds")
rm(UTx01_Bx01)
gc()
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx02/UTx01_Bx02_doublet_predictions.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx02/UTx01_Bx02_doublet_predictions_histogram.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```

```{r out.height = "460px", out.width='800px', echo=F, eval=FALSE}
knitr::include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx02/UTx01_Bx02_doublet_predictions_histogram.pdf')
```

```{r}
#Loading scrublet predictions
dim(scrublet_calls <- read.csv("/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx01_Bx02/UTx01_Bx02_scrublet_calls.csv"))
table(scrublet_calls$predicted_doublets)
...
### Saving object for further downstream analysis
```{r}
saveRDS(UTx01_Bx02, file = "/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/QC/UTx01_Bx02_SeuratObject_soupX_doublet_QC_filtered.rds")
rm(UTx01_Bx02)
gc()
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx02_Bx02/UTx02_Bx02_doublet_predictions.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx02_Bx02/UTx02_Bx02_doublet_predictions_histogram.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```

```{r out.height = "460px", out.width='800px', echo=F, eval=FALSE}
knitr::include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx02_Bx02/UTx02_Bx02_doublet_predictions_histogram.pdf')
```

```{r}
#Loading scrublet predictions
dim(scrublet_calls <- read.csv("/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx02_Bx02/UTx02_Bx02_scrublet_calls.csv"))
table(scrublet_calls$predicted_doublets)
...
### Saving object for further downstream analysis
```{r}
saveRDS(UTx02_Bx02, file = "/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/QC/UTx02_Bx02_SeuratObject_soupX_doublet_QC_filtered.rds")
rm(UTx02_Bx02)
gc()
```
...
```{r, fig.height=7, fig.width=13}
#We'll store the percentage of reads that map to the mitochondrial genome in the metadata object as "percent.mt"
UTx03_Bx04[["percent.mt"]] <- PercentageFeatureSet(UTx03_Bx04, pattern = "^MT-")

#The number of unique genes and total molecules are automatically calculated during `CreateSeuratObject()` and we can find these stored in the object meta data as nFeature_RNA and nCount_RNA respecitvely.
UTx03_Bx04@meta.data %>%
  head(n=5)
```
...
```{r, message=FALSE, fig.height=4}
#Visualize QC metrics as violin plots
p1 <- VlnPlot(UTx03_Bx04, features = c("nFeature_RNA"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
p2 <- VlnPlot(UTx03_Bx04, features = c("nCount_RNA"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
p3 <- VlnPlot(UTx03_Bx04, features = c("percent.mt"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
grid.arrange(p1, p2, p3, ncol=3)
```
...
```{r, message=FALSE, fig.width=20, fig.height=7}
#FeatureScatter is typically used to visualize feature-feature relationships, but can be used for anything calculated by the object, i.e. columns in object metadata, PC scores etc.
plot1 <- FeatureScatter(UTx03_Bx04, feature1 = "nCount_RNA", feature2 = "percent.mt", cols = "gray60")
plot2 <- FeatureScatter(UTx03_Bx04, feature1 = "nCount_RNA", feature2 = "nFeature_RNA", cols = "gray60")
plot1 + plot2
```

**Alternative view of the data and distributions**
```{r, fig.height=3}
df <- as.data.table(UTx03_Bx04@meta.data)
sel <- c("orig.ident", "nCount_RNA", "nFeature_RNA", "percent.mt")
df <- df[, sel, with = FALSE]
df[1:3, ]
fontsize <- 10
...
### More on Data and QC
```{r, message=FALSE, warning=FALSE, fig.height=4, fig.width=6}
counts <- Matrix(UTx03_Bx04@assays$RNA@counts)
counts_per_cell <- Matrix::colSums(counts)
counts_per_gene <- Matrix::rowSums(counts)
genes_per_cell1 <- Matrix::colSums(counts>0) #count a gene only if it has non-zero reads mapped.
cells_per_gene <- Matrix::rowSums(counts>0) #only count cells where the gene is expressed
...
```{r fig.width=12, fig.height=6}
#Density plot
ggplot(UTx03_Bx04@meta.data, aes(x=UTx03_Bx04@meta.data$percent.mt)) +
  geom_density() + scale_color_manual(values=c("blue")) + theme_classic() +
  geom_vline(aes(xintercept=mean(UTx03_Bx04@meta.data$percent.mt)),
             color="blue", linetype="dashed", size=0.5) +scale_x_continuous(breaks=seq(0, 40, by = 1))
```
...
```{r}
(Count93_nCount_RNA <- quantile(UTx03_Bx04@meta.data$nCount_RNA, 0.93)) # calculate value in the 93rd percentile for a hint on thresholds but these should be taken with a grain of salt, look at the above plots as well to determine thresholds
(Count93_nFeature_RNA <- quantile(UTx03_Bx04@meta.data$nFeature_RNA, 0.93))
(Count93_percent_mt <-  quantile(UTx03_Bx04@meta.data$percent.mt, 0.93))

summary(UTx03_Bx04@meta.data$nCount_RNA)
summary(UTx03_Bx04@meta.data$nFeature_RNA)

UTx03_Bx04 <- subset(UTx03_Bx04, subset = nFeature_RNA > 200 & nFeature_RNA < 3500 & percent.mt < 15)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx03_Bx04/UTx03_Bx04_doublet_predictions.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx03_Bx04/UTx03_Bx04_doublet_predictions_histogram.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```

```{r out.height = "460px", out.width='800px', echo=F, eval=FALSE}
knitr::include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx03_Bx04/UTx03_Bx04_doublet_predictions_histogram.pdf')
```

```{r}
#Loading scrublet predictions
dim(scrublet_calls <- read.csv("/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx03_Bx04/UTx03_Bx04_scrublet_calls.csv"))
table(scrublet_calls$predicted_doublets)

scrublet_calls$X <- paste0("Late_Secretory_UTx03_Bx04_", scrublet_calls$X)

dim(scrublet_calls <- scrublet_calls[which(scrublet_calls$X %in% rownames(UTx03_Bx04@meta.data)),])
rownames(scrublet_calls) <- scrublet_calls$X
scrublet_calls$X <-NULL
dim(scrublet_calls)

#Adding doublet information to metadata
#First we'll ensure that the rownames in UTx03_Bx04 match the rownames in scrublet_calls. AddMetaData maps rownames but we'll still do so to ensure that mapping of predictions are made to respective bar codes
scrublet_calls <- scrublet_calls[rownames(UTx03_Bx04@meta.data), ]
head(rownames(scrublet_calls))
head(rownames(UTx03_Bx04@meta.data))
UTx03_Bx04 <- AddMetaData(UTx03_Bx04, scrublet_calls)
```


```{r, fig.height=4, fig.width=5}
#Without normalizing the data, we want to first visualize the doublets in our datasets
UTx03_Bx04_Control_2 <- UTx03_Bx04
UTx03_Bx04_Control_2 <- FindVariableFeatures(UTx03_Bx04_Control_2, selection.method = "vst", nfeatures = 2500)
UTx03_Bx04_Control_2 <- ScaleData(object = UTx03_Bx04_Control_2, scale.max = 30,  verbose = FALSE)
UTx03_Bx04_Control_2 <- RunPCA(object = UTx03_Bx04_Control_2, npcs = 30, verbose = FALSE)
UTx03_Bx04_Control_2 <- FindNeighbors(UTx03_Bx04_Control_2, dims = 1:20, verbose = TRUE, reduction = "pca")
UTx03_Bx04_Control_2 <- RunUMAP(UTx03_Bx04_Control_2, dims = 1:20, verbose = TRUE, reduction = "pca")
UTx03_Bx04_Control_2 <- FindClusters(UTx03_Bx04_Control_2, verbose = TRUE, reduction = "pca") #Resolution can be adjusted - leaving to default for now in test dataset

FeaturePlot(UTx03_Bx04_Control_2, features = "doublet_scores", pt.size = 0.01)

DimPlot(UTx03_Bx04_Control_2, group.by = "predicted_doublets", pt.size = 0.01, cols = c("gray90", "firebrick3"))

#Checking the nUMI for doublets and singlets
VlnPlot(UTx03_Bx04_Control_2,
        features = "nCount_RNA",
        pt.size = 0,
        group.by = "predicted_doublets") + NoLegend()

#Fractions of doublets per cluster
df <- data.table(UTx03_Bx04_Control_2@meta.data)
...
```{r, fig.height=4, fig.width=5}
#Next we'll remove the doublets and see what the data looks like
UTx03_Bx04_Control_2 <- UTx03_Bx04_Control_2[, UTx03_Bx04_Control_2@meta.data[, "predicted_doublets"] == "False"]
unique(UTx03_Bx04_Control_2@meta.data$predicted_doublets)
DimPlot(UTx03_Bx04_Control_2, group.by = "predicted_doublets", pt.size = 0.01, cols = c("gray90", "firebrick3"), label = TRUE)
```

**Filtering cells to remove doublets**
```{r}
UTx03_Bx04 <- UTx03_Bx04[, UTx03_Bx04@meta.data[, "predicted_doublets"] == "False"]
unique(UTx03_Bx04@meta.data$predicted_doublets)
dim(UTx03_Bx04)
```
...
### Saving object for further downstream analysis
```{r}
saveRDS(UTx03_Bx04, file = "/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/QC/UTx03_Bx04_SeuratObject_soupX_doublet_QC_filtered.rds")
rm(UTx03_Bx04)
gc()
```
...
#We'll store the percentage of reads that map to the mitochondrial genome in the metadata object as "percent.mt"
UTx04_Bx01[["percent.mt"]] <- PercentageFeatureSet(UTx04_Bx01, pattern = "^MT-")

#The number of unique genes and total molecules are automatically calculated during `CreateSeuratObject()` and we can find these stored in the object meta data as nFeature_RNA and nCount_RNA respecitvely.
UTx04_Bx01@meta.data %>% 
  head(n=5)
```
...
```{r, message=FALSE, fig.height=4}
#Visualize QC metrics as violin plots
p1 <- VlnPlot(UTx04_Bx01, features = c("nFeature_RNA"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
p2 <- VlnPlot(UTx04_Bx01, features = c("nCount_RNA"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
p3 <- VlnPlot(UTx04_Bx01, features = c("percent.mt"), ncol = 1, cols = "skyblue") + theme_light(base_size = 14) + theme(legend.position = "none", plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
grid.arrange(p1, p2, p3, ncol=3)
```
...
```{r, message=FALSE, fig.width=20, fig.height=7}
#FeatureScatter is typically used to visualize feature-feature relationships, but can be used for anything calculated by the object, i.e. columns in object metadata, PC scores etc.
plot1 <- FeatureScatter(UTx04_Bx01, feature1 = "nCount_RNA", feature2 = "percent.mt", cols = "gray60") 
plot2 <- FeatureScatter(UTx04_Bx01, feature1 = "nCount_RNA", feature2 = "nFeature_RNA", cols = "gray60") 
plot1 + plot2
```

**Alternative view of the data and distributions**
```{r, fig.height=3}
df <- as.data.table(UTx04_Bx01@meta.data)
sel <- c("orig.ident", "nCount_RNA", "nFeature_RNA", "percent.mt")
df <- df[, sel, with = FALSE]
df[1:3, ]
fontsize <- 10
...
### More on Data and QC
```{r, message=FALSE, warning=FALSE, fig.height=4, fig.width=6}
counts <- Matrix(UTx04_Bx01@assays$RNA@counts)
counts_per_cell <- Matrix::colSums(counts)
counts_per_gene <- Matrix::rowSums(counts)
genes_per_cell1 <- Matrix::colSums(counts>0) #count a gene only if it has non-zero reads mapped.
cells_per_gene <- Matrix::rowSums(counts>0) #only count cells where the gene is expressed
...
```{r fig.width=12, fig.height=6}
#Density plot
ggplot(UTx04_Bx01@meta.data, aes(x=UTx04_Bx01@meta.data$percent.mt)) +
  geom_density() + scale_color_manual(values=c("blue")) + theme_classic() +
  geom_vline(aes(xintercept=mean(UTx04_Bx01@meta.data$percent.mt)),
             color="blue", linetype="dashed", size=0.5) +scale_x_continuous(breaks=seq(0, 40, by = 1)) 
```
...
```{r}
(Count93_nCount_RNA <- quantile(UTx04_Bx01@meta.data$nCount_RNA, 0.93)) # calculate value in the 93rd percentile for a hint on thresholds but these should be taken with a grain of salt, look at the above plots as well to determine thresholds
(Count93_nFeature_RNA <- quantile(UTx04_Bx01@meta.data$nFeature_RNA, 0.93))
(Count93_percent_mt <-  quantile(UTx04_Bx01@meta.data$percent.mt, 0.93))

summary(UTx04_Bx01@meta.data$nCount_RNA)
summary(UTx04_Bx01@meta.data$nFeature_RNA)

UTx04_Bx01 <- subset(UTx04_Bx01, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 10) 
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx04_Bx01/UTx04_Bx01_doublet_predictions.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```
...
```{r, out.width = '70%', include=TRUE, fig.align="center", fig.cap=c("scrublet_predictions")}
include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx04_Bx01/UTx04_Bx01_doublet_predictions_histogram.pdf', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```

```{r out.height = "460px", out.width='800px', echo=F, eval=FALSE}
knitr::include_graphics('/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx04_Bx01/UTx04_Bx01_doublet_predictions_histogram.pdf')
```

```{r}
#Loading scrublet predictions
dim(scrublet_calls <- read.csv("/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/scrublet_predictions/UTx04_Bx01/UTx04_Bx01_scrublet_calls.csv")) 
table(scrublet_calls$predicted_doublets)

scrublet_calls$X <- paste0("Menses_UTx04_Bx01_", scrublet_calls$X)

dim(scrublet_calls <- scrublet_calls[which(scrublet_calls$X %in% rownames(UTx04_Bx01@meta.data)),])
rownames(scrublet_calls) <- scrublet_calls$X
scrublet_calls$X <-NULL
dim(scrublet_calls)

#Adding doublet information to metadata
#First we'll ensure that the rownames in UTx04_Bx01 match the rownames in scrublet_calls. AddMetaData maps rownames but we'll still do so to ensure that mapping of predictions are made to respective bar codes
scrublet_calls <- scrublet_calls[rownames(UTx04_Bx01@meta.data), ]
head(rownames(scrublet_calls))
head(rownames(UTx04_Bx01@meta.data))
UTx04_Bx01 <- AddMetaData(UTx04_Bx01, scrublet_calls)
```


```{r, fig.height=4, fig.width=5}
#Without normalizing the data, we want to first visualize the doublets in our datasets
UTx04_Bx01_Control_2 <- UTx04_Bx01
UTx04_Bx01_Control_2 <- FindVariableFeatures(UTx04_Bx01_Control_2, selection.method = "vst", nfeatures = 2500)
UTx04_Bx01_Control_2 <- ScaleData(object = UTx04_Bx01_Control_2, scale.max = 30,  verbose = FALSE)
UTx04_Bx01_Control_2 <- RunPCA(object = UTx04_Bx01_Control_2, npcs = 30, verbose = FALSE)
UTx04_Bx01_Control_2 <- FindNeighbors(UTx04_Bx01_Control_2, dims = 1:20, verbose = TRUE, reduction = "pca")
UTx04_Bx01_Control_2 <- RunUMAP(UTx04_Bx01_Control_2, dims = 1:20, verbose = TRUE, reduction = "pca")
UTx04_Bx01_Control_2 <- FindClusters(UTx04_Bx01_Control_2, verbose = TRUE, reduction = "pca") #Resolution can be adjusted - leaving to default for now in test dataset

FeaturePlot(UTx04_Bx01_Control_2, features = "doublet_scores", pt.size = 0.01)

DimPlot(UTx04_Bx01_Control_2, group.by = "predicted_doublets", pt.size = 0.01, cols = c("gray90", "firebrick3"))

#Checking the nUMI for doublets and singlets
VlnPlot(UTx04_Bx01_Control_2,
        features = "nCount_RNA",
        pt.size = 0,
        group.by = "predicted_doublets") + NoLegend()

#Fractions of doublets per cluster
df <- data.table(UTx04_Bx01_Control_2@meta.data)
...
```{r, fig.height=4, fig.width=5}
#Next we'll remove the doublets and see what the data looks like
UTx04_Bx01_Control_2 <- UTx04_Bx01_Control_2[, UTx04_Bx01_Control_2@meta.data[, "predicted_doublets"] == "False"]
unique(UTx04_Bx01_Control_2@meta.data$predicted_doublets)
DimPlot(UTx04_Bx01_Control_2, group.by = "predicted_doublets", pt.size = 0.01, cols = c("gray90", "firebrick3"), label = TRUE)
```

**Filtering cells to remove doublets**
```{r}
UTx04_Bx01 <- UTx04_Bx01[, UTx04_Bx01@meta.data[, "predicted_doublets"] == "False"]
unique(UTx04_Bx01@meta.data$predicted_doublets)
dim(UTx04_Bx01)
```
...
### Saving object for further downstream analysis
```{r}
saveRDS(UTx04_Bx01, file = "/data/project/porrettlab/lab_members/rasiimwe/HC_UTx_sample_analytics/QC/UTx04_Bx01_SeuratObject_soupX_doublet_QC_filtered.rds")
rm(UTx04_Bx01) 
gc()
```
